...
بی‌رمز روی Mac می‌ماند — و با root وارد می‌شود. برای همین سرور آن را پشتِ
`rrsync -ro ~/backups/` قفل می‌کند: فقط **خواندن از پوشه‌ی بکاپ**. نه shell، نه اجرای
دستور، نه نوشتن، نه خواندنِ فایلِ دیگر (هر سه آزمایش شده‌اند). اگر Mac دزدیده شود،
بدترین اتفاق لو رفتنِ همان بکاپ‌هاست، نه سرور.
